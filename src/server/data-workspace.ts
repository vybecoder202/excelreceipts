...
export async function getDataWorkspace(projectId: string) {
  ...
  const [tables, fields, records, cells, links, comments, views, forms, formFields, interfaces, blocks] = await Promise.all([
    ...
    supabase.from("data_records").select("id, table_id, record_number, position, created_at, updated_at").eq("project_id", projectId).is("archived_at", null).order("position").order("record_number"),
    ...
    supabase.from("data_record_comments").select("id, table_id, record_id, body, created_at").eq("project_id", projectId).order("created_at"),
  ...
  const firstError = [tables, fields, records, cells, links, comments, views, forms, formFields, interfaces, blocks].find((result) => result.error)?.error;
