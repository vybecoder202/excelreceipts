...
type TaskRow = Pick<Tables<"tasks">, "id" | "phase_id" | "status">;
...
export default async function SiteProgressPage({
  ...
  const [phaseResult, taskResult, milestoneResult, progressResult] = await Promise.all([
    supabase
      .from("phases")
      .select("*")
  ...
  const dataError = Boolean(
    phaseResult.error || taskResult.error || milestoneResult.error || progressResult.error,
  );
    ...
    <div className="space-y-6 lg:space-y-8">
      {created === "phase" ? (
        <div className="flex items-start gap-3 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm leading-6 text-emerald-950" role="status">
          ...
          <div>
            <p className="font-bold">Phase created</p>
            <p>The phase reference, audit event, and project sequence were recorded together.</p>
          </div>
        </div>
      ) : null}
